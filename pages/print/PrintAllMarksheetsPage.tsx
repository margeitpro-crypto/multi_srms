...
const SingleMarksheet: React.FC<{ studentId: string }> = ({ studentId }) => {
    ...
    if (!student || !school) {
        return (
            <div className="marksheet-container bg-white p-4 my-8 text-center text-red-500 border-2 border-dashed border-red-400">
    ...
    return (
        <div className="marksheet-container bg-white p-2 relative shadow-2xl print:shadow-none" id={`marksheet-${student.id}`}>
            <div className="border-8 border-gray-800 p-4 relative overflow-hidden" style={{borderStyle: 'double'}}>
                <div className="absolute inset-0 overflow-hidden pointer-events-none z-0" aria-hidden="true">
                    <div
                        className="p-2 text-gray-600 font-thin uppercase"
                        style={{
                            fontSize: '0.7rem',
                            lineHeight: 1.5,
                            wordBreak: 'break-all',
...
const PrintAllMarksheetsPage: React.FC = () => {
    ...
    if (!studentIds || studentIds.length === 0) {
        return (
            <div className="p-8 text-center">
                ...
                {/* FIX: Replaced `Link` with a `Button` using `useNavigate` for programmatic navigation. */}
            ...
            <div className="max-w-4xl mx-auto">
                <div className="mb-6 flex justify-between items-center print:hidden">
                    ...
                    <Button onClick={() => window.print()} leftIcon={<PrinterIcon className="w-5 h-5" />}>
                        Print All ({studentIds.length}) Grade Sheets
                    </Button>
                </div>

                <div className="space-y-8">
                    {studentIds.map(id => <SingleMarksheet key={id} studentId={id} />)}
                </div>
            </div>
        </div>
    );
...
export default PrintAllMarksheetsPage;